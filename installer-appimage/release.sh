...
TAG="triple-rice-installer-v1.0.1"
TITLE="Huzaifah Triple-Rice Installer v1.0.1"
DIST="$ROOT/dist"
...
cat > "$NOTES" <<'EOF'
# Huzaifah Triple-Rice Installer v1.0.1
...
The launcher now downloads `install.sh` to a temporary file, validates its shell syntax, and executes it only after a successful download. Failed downloads can no longer be reported as successful installations.
